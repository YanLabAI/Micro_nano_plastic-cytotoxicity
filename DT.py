...
# Build the model
Xtrain, Xtest, Ytrain, Ytest = train_test_split(x0, y, test_size=0.2, random_state=257)
'''
...
for i in range(0, 200, 1):
    ...
    score_5cv = cross_val_score(dt, Xtrain, Ytrain, cv=5).mean()
...
CV_score = cross_val_score(dt, Xtrain, Ytrain, cv=5).mean()
CV_predictions = cross_val_predict(dt, Xtrain,Ytrain,cv=5)
...
pd.DataFrame(expvspred_5cv).to_excel('/home/chl/ML_new/DT/DT_5fcv_pred.xlsx')

'''
Test set validation

'''
regressor = dt.fit(Xtrain, Ytrain)
...
pd.DataFrame(expvspred_test).to_excel('/home/chl/ML_new/DT/DT_test_pred.xlsx')